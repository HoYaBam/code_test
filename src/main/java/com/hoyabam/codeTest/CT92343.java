...
/*
    ...
    각 노드를 방문할 때 마다 해당 노드에 있던 양과 늑대가 당신을 따라오게 됩니다. 이때, 늑대는 양을 잡아먹을 기회를 노리고 있으며,
    ...
    이어서 늑대가 있는 노드로 이동한다면(예를 들어 바로 6번 노드로 이동한다면) 양 2마리, 늑대 2마리가 되어 양이 모두 잡아먹힙니다. 여기서는 0번,
    1번 노드를 방문하여 양을 2마리 모은 후, 8번 노드로 이동한 후(양 2마리 늑대 1마리) 이어서 7번, 9번 노드를 방문하면 양 4마리 늑대 1마리가 됩니다.
...
public class CT92343 {
    ...
    public static void main(String[] args) {
        ...
        bfs(0, 1, 0, nextList, info, edges);
        System.out.println(maxSheep);
    }

    private static void bfs(int index, int sheep, int wolf, ArrayList<Integer> nextList, int[] info, int[][] edges) {
        maxSheep = Math.max(maxSheep, sheep);

        for (int i = 0; i < edges.length; i++) {
            if (edges[i][0] == index) {
                nextList.add(edges[i][1]);
            }
        }
        ...
        for (int i = 0; i < nextList.size(); i++) {
            ...
            if (info[nextNode] == 0) newSheep++;
            else newWolf++;

            if (newWolf >= newSheep) continue;

            ArrayList<Integer> newList = new ArrayList<>(nextList);
            newList.remove(i);

            bfs(nextNode, newSheep, newWolf, newList, info, edges);
        }
    }
}