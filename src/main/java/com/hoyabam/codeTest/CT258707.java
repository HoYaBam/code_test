...
public class CT258707 {
    ...
    public static void main(String[] args) {
        ...
        int count = 1;
        for (int i = fn; i < n; i += 2) {
            dummyList.add(cards[i]);
            dummyList.add(cards[i + 1]);
            ...
            for (int j = 0; j < cardsList.size(); j++) {
                for (int k = j + 1; k < cardsList.size(); k++) {
                    if (cardsList.get(j) + cardsList.get(k) == target) {
                        cardsList.remove(Math.max(j, k));
                        cardsList.remove(Math.min(j, k));
                        count++;
                        next = true;
                        break outer1;
                    }
            ...
            if (!next && coin > 0) {
                outer2:
                for (int d = 0; d < dummyList.size(); d++) {
                    for (int k = 0; k < cardsList.size(); k++) {
                        if (dummyList.get(d) + cardsList.get(k) == target) {
                            dummyList.remove(d);
                            cardsList.remove(k);
                            coin--;
                            count++;
            ...
            if (!next && coin >= 2) {
                outer3:
                for (int d = 0; d < dummyList.size(); d++) {
                    for (int k = d + 1; k < dummyList.size(); k++) {
                        if (dummyList.get(d) + dummyList.get(k) == target) {
                            dummyList.remove(Math.max(d, k));
                            dummyList.remove(Math.min(d, k));
                            coin -= 2;
                            count++;
                            next = true;
                            break outer3;
